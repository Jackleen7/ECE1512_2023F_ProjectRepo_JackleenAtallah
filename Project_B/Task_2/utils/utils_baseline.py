...
def epoch(mode, dataloader, net, optimizer, criterion, dataset, device = 'cuda'):
    ...
    if dataset == 'MNIST':
        ...
        f1score = f1_score(np.hstack(true_labels), np.hstack(pred_labels))
        return loss_avg, f1score
...
def evaluate_synset(net, images_train, labels_train, testloader, 
    ...
    _, labels_train = torch.max(labels_train.data, 1)
    labels_train = labels_train.to(device)
    lr = float(args.lr_net)
    Epoch = int(args.epoch_eval_train)
    optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.5)
    criterion = nn.CrossEntropyLoss()
    lr_schedule = CosineAnnealingLR(optimizer, T_max=Epoch)

    dst_train = TensorDataset(images_train, labels_train)
    trainloader = torch.utils.data.DataLoader(dst_train, batch_size=args.batch_syn, 
                                              shuffle=True, num_workers=0)

    start = time.time()
    for ep in range(Epoch+1):
        loss_train, score_train = epoch('train', trainloader, net, optimizer, criterion, args.dataset)        
        lr_schedule.step()
    time_train = time.time() - start
    loss_test, score_test = epoch('test', testloader, net, optimizer, criterion, args.dataset)
    ...
    if args.dataset == 'MNIST':
        ...
        print('%s Evaluate: epoch = %04d train time = %.4f s train loss = %.6f train f1 = %.4f, test f1 = %.4f' % (get_time(), 
                                                        Epoch, time_train, loss_train, score_train, score_test))
