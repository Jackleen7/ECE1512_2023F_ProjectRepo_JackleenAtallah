...
from networks import MLP, ConvNet, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, \
      VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN, VGG11_mhist
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    elif model == 'VGG11_mhist':
        net = VGG11_mhist( channel=channel, num_classes=num_classes)
    elif model == 'VGG11BN':
...
def match_loss(gw_syn, gw_real, dis_metric, device = 'cuda'):
...
def epoch(mode, dataloader, net, optimizer, criterion, dataset, device = 'cuda',
          aug = False):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if aug:
           img = DiffAugment(img, 'color_crop_scale_rotate', param = ParamDiffAug())
        ...
        if dataset == 'MNIST':
            ...
            pred_labels.append(np.argmax(output.cpu().data.numpy(), axis=-1))
            true_labels.append(lab.cpu().data.numpy())
    ...
    if dataset == 'MNIST':
        ...
        hp_f1score = f1_score(np.hstack(true_labels), np.hstack(pred_labels), pos_label=0)
        ssa_f1score = f1_score(np.hstack(true_labels), np.hstack(pred_labels), pos_label=1)
        wf1score = f1_score(np.hstack(true_labels), np.hstack(pred_labels), average = 'weighted')
        return loss_avg, (hp_f1score, ssa_f1score, wf1score)
...
def evaluate_synset(net, images_train, labels_train, testloader, 
                    batch_train, lr_net, epoch_eval_train, dataset, device = 'cuda'):
    net = net.to(device)
    ...
    lr_schedule = CosineAnnealingLR(optimizer, T_max=epoch_eval_train)
    ...
    if dataset == 'MNIST':
        ...
        print('%s Evaluate: epoch = %04d train time = %.4f s train loss = %.6f' % (get_time(), Epoch, time_train, loss_train))
        print('Train Score: HP_F1Score: %.4f, SSA_F1Score: %.4f, weighted_F1Score: %.4f'%(score_train[0], score_train[1], score_train[2]))
        print('Test Score: HP_F1Score: %.4f, SSA_F1Score: %.4f, weighted_F1Score: %.4f'%(score_test[0], score_test[1], score_test[2]))
    return net, score_train, score_test
